...
func Test_push_should_not_override_lastReadTo_before_wrap(t *testing.T) {
	...
	assert(buffer.data, "==", []byte{
		2, 0, byte('C'), byte('C'), // 3rd packet
		0, byte('B'), byte('B'), // 2nd packet
		0, 0, 0,
	})
...
func Test_wrap_then_push_pop_pop(t *testing.T) {
	assert := NewAssert(t)
	buffer := newBuffer(10)
	buffer.PushN([][]byte{
		[]byte("A"),
